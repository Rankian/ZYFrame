...
import com.sanjie.zy.utils.IdUtils;
import com.sanjie.zy.utils.ZYDisplayUtils;
import com.sanjie.zy.widget.indicatior.animation.AbsAnimation;
import com.sanjie.zy.widget.indicatior.animation.AnimationType;
import com.sanjie.zy.widget.indicatior.animation.ColorAnimation;
import com.sanjie.zy.widget.indicatior.animation.FillAnimation;
import com.sanjie.zy.widget.indicatior.animation.ScaleAnimation;
import com.sanjie.zy.widget.indicatior.animation.ValueAnimation;

/**
 * Created by SanJie on 2017/5/16.
 */
public class ZYPageIndicatorView extends View implements ViewPager.OnPageChangeListener {
    ...
    public ZYPageIndicatorView(Context context) {
        super(context);
        init(null);
    }

    public ZYPageIndicatorView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public ZYPageIndicatorView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public ZYPageIndicatorView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        positionSavedState.setSelectedIndex(selectedPosition);
        positionSavedState.setSelectingIndex(selectingPosition);
        positionSavedState.setLastSelectedIndex(lastSelectedPosition);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof PositionSavedState) {
            ...
            this.selectedPosition = positionSavedState.getSelectedIndex();
            this.selectingPosition = positionSavedState.getSelectingIndex();
            this.lastSelectedPosition = positionSavedState.getLastSelectedIndex();
    ...
    /**
     * Set radius in dp of each circle indicator. Default value is {@link ZYPageIndicatorView#DEFAULT_RADIUS_DP}.
    ...
    public void setRadius(int radiusDp) {
        ...
        radiusPx = ZYDisplayUtils.dp2px(radiusDp);
        invalidate();
    }

    /**
     * Set radius in px of each circle indicator. Default value is {@link ZYPageIndicatorView#DEFAULT_RADIUS_DP}.
     * Note: make sure you set circle Radius, not a Diameter.
     *
     ...
     * default value {@link ZYPageIndicatorView#DEFAULT_RADIUS_DP}.
     */
    ...
    /**
     * Set padding in dp between each circle indicator. Default value is {@link ZYPageIndicatorView#DEFAULT_PADDING_DP}.
     *
    ...
    public void setPadding(int paddingDp) {
        ...
        paddingPx = ZYDisplayUtils.dp2px(paddingDp);
        invalidate();
    }

    /**
     * Set padding in px between each circle indicator. Default value is {@link ZYPageIndicatorView#DEFAULT_PADDING_DP}.
     *
     ...
     * return default value {@link ZYPageIndicatorView#DEFAULT_PADDING_DP}.
     */
    ...
    public void setStrokeWidth(int strokeDp) {
        int strokePx = ZYDisplayUtils.dp2px(strokeDp);
     ...
     * One of {@link Mode}: Off (Left to right), On (Right to left)
     * or Auto (handle this mode automatically based on users language preferences).
     * Default is Off.
     *
     * @param mode instance of {@link Mode}
     */
    public void setRtlMode(@Nullable Mode mode) {
        if (mode == null) {
            mode = Mode.OFF;
        } else {
            mode = mode;
    ...
    private void initAttributes(@Nullable AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.ZYPageIndicatorView, 0, 0);
    ...
    private void initCountAttribute(@NonNull TypedArray typedArray) {
        autoVisibility = typedArray.getBoolean(R.styleable.ZYPageIndicatorView_autoVisibility, true);
        dynamicCount = typedArray.getBoolean(R.styleable.ZYPageIndicatorView_dynamicCount, false);
        count = typedArray.getInt(R.styleable.ZYPageIndicatorView_count, COUNT_NOT_SET);
        ...
        int position = typedArray.getInt(R.styleable.ZYPageIndicatorView_select, 0);
        if (position < 0) {
            position = 0;
        ...
        viewPagerId = typedArray.getResourceId(R.styleable.ZYPageIndicatorView_viewPager, 0);
    }

    private void initColorAttribute(@NonNull TypedArray typedArray) {
        unselectedColor = typedArray.getColor(R.styleable.ZYPageIndicatorView_unselectedColor, Color.parseColor(ColorAnimation.DEFAULT_UNSELECTED_COLOR));
        selectedColor = typedArray.getColor(R.styleable.ZYPageIndicatorView_selectedColor, Color.parseColor(ColorAnimation.DEFAULT_SELECTED_COLOR));
    }

    private void initAnimationAttribute(@NonNull TypedArray typedArray) {
        animationDuration = typedArray.getInt(R.styleable.ZYPageIndicatorView_animationDuration, AbsAnimation.DEFAULT_ANIMATION_DURATION);
        interactiveAnimation = typedArray.getBoolean(R.styleable.ZYPageIndicatorView_interactiveAnimation, false);
    ...
    private void initSizeAttribute(@NonNull TypedArray typedArray) {
        int orientationIndex = typedArray.getInt(R.styleable.ZYPageIndicatorView_orientation, Orientation.HORIZONTAL.ordinal());
        ...
        radiusPx = (int) typedArray.getDimension(R.styleable.ZYPageIndicatorView_radius, ZYDisplayUtils.dp2px(DEFAULT_RADIUS_DP));
        paddingPx = (int) typedArray.getDimension(R.styleable.ZYPageIndicatorView_padding, ZYDisplayUtils.dp2px(DEFAULT_PADDING_DP));

        scaleFactor = typedArray.getFloat(R.styleable.ZYPageIndicatorView_scaleFactor, ScaleAnimation.DEFAULT_SCALE_FACTOR);
        ...
        strokePx = (int) typedArray.getDimension(R.styleable.ZYPageIndicatorView_strokeWidth, ZYDisplayUtils.dp2px(FillAnimation.DEFAULT_STROKE_DP));
